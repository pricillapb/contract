...
func TestReadNodesCycle(t *testing.T) {
	iter := &callCountIter{
		Iterator: CycleNodes([]*enode.Node{
			testNode(0, 0),
			testNode(1, 0),
			testNode(2, 0),
		}),
...
func TestFilterNodes(t *testing.T) {
	nodes := make([]*enode.Node, 100)
	for i := range nodes {
		nodes[i] = testNode(uint64(i), uint64(i))
	}

	it := Filter(IterNodes(nodes), func(n *enode.Node) bool {
		return n.Seq() >= 50
	})
...
func TestFairMixEmpty(t *testing.T) {
	...
	mix.AddSource(CycleNodes([]*enode.Node{testN}))
	if n := <-ch; n != testN {
...
func TestFairMixRemoveSource(t *testing.T) {
	...
	mix.AddSource(CycleNodes([]*enode.Node{wantNode}))
	n := <-sig
...
func (it blockingIter) Node() *enode.Node {
	return nil
}
