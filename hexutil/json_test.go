...
var bytesUmarshalTests = []unmarshalTest{
	// invalid encoding
...
func TestUnmarshalBytes(t *testing.T) {
	for _, test := range bytesUmarshalTests {
		var v Bytes
...
var bigUnmarshalTests = []unmarshalTest{
	// invalid encoding
	{input: "", wantErr: errNonString},
	{input: "null", wantErr: errNonString},
	{input: "10", wantErr: errNonString},
...
func TestUnmarshalBig(t *testing.T) {
	for _, test := range bigUnmarshalTests {
		var v Big
		err := v.UnmarshalJSON([]byte(test.input))
		if !checkError(t, test.input, err, test.wantErr) {
			continue
...
var uint64UnmarshalTests = []unmarshalTest{
	// invalid encoding
	{input: "", wantErr: errNonString},
	{input: "null", wantErr: errNonString},
	{input: "10", wantErr: errNonString},
...
func TestUnmarshalUint(t *testing.T) {
	for _, test := range uint64UnmarshalTests {
		var v Uint
		err := v.UnmarshalJSON([]byte(test.input))
		if !checkError(t, test.input, err, test.wantErr) {
			continue
