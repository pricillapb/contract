...
func NewLesServer(eth *eth.Ethereum, config *eth.Config) (*LesServer, error) {
	...
	pm, err := NewProtocolManager(eth.BlockChain().Config(), false, ProtocolVersions, config.NetworkId, eth.EventMux(), eth.Engine(), newPeerSet(), eth.BlockChain(), eth.TxPool(), eth.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	if chtV1SectionCount != 0 {
		// convert to LES/2 section
		chtLastSection := chtV1SectionCount/(light.ChtFrequency/light.ChtV1Frequency) - 1
