...
// CreateAccount explicitly creates a state object. If a state object whith the address
// already exists the balance is caried over to the new account.
//
...
func (self *StateDB) RevertToSnapshot(snapshotId int) {
	...
	idx := sort.Search(len(self.validSnapshots), func(i int) bool {
		return self.validSnapshots[i].id >= snapshotId
	})
	if idx == len(self.validSnapshots) || self.validSnapshots[idx].id != snapshotId {
		panic(fmt.Errorf("snapshot id %v cannot be reverted", snapshotId))
	}
...
func (s *StateDB) clearJournal() {
	s.journal = nil
	s.validSnapshots = s.validSnapshots[:0]
}
